...
Png_Tube2:TpngImage;
Png_Stars:TpngImage;
//переменная ,в которой кол-во очков.
score:integer;
//флаг кот. проверяет пролетели мы через трубу или нет.
flag:boolean;
// переменная скорости.
V:integer;
...
begin
// выставляет нулевую начальную скорость.
V:=0;
//делаем так,чтобы картинки не мерцали при двежении.
Form1.DoubleBuffered:=true;
//загружаем в картинки изображение из ресурсов.
...
Png_Ground.Free;
//разьещаем землю одну за другой.
...
Png_Stars.Free;
 ...
 //если нажата клавиша ПРОБЕЛ то ускоряем нашу птичку с силой 10.
...
//осноыной таймер в котором содержится логика всего что есть в игре.
...
begin
   ...
   Image7.Left:=Image7.Left -2;
    ...
    Image9.Left:=Image8.Left;
         ...
         // выстовлени и телепортация труб.
...
//физика падения птички.
  ...
  //недаёт птичке улететь выше верхней границы.
  if (Image1.Top <0) then
  begin
   Image1.Top:=0;
   V:=0;
  end;
   //если птичка упала вниз то возвращаем её вверх.
  ...
  //сталкновение птички с  нижней трубой.
  ...
  begin
   ...
   //сталкновение птички с верхней трубой.
